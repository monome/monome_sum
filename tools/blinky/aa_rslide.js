...
function draw() {
	...
	if(lef>lif) {
		...
		them[lef] = -Math.floor(5*(lef-le));
	}
	else if(lif>lef) {
		for(i=lef;i<lif;i++) them[i] = 5;
		them[lef] = 5-Math.floor(-5*(lef-le));
	}


	them[lif+1] = Math.max(-Math.floor(15*(lif-li)),them[lif]); // .01 is to avoid weird overflow edge case
	them[lif] = 15;
	them[lif-1] = Math.max(15-(-Math.floor(15*(lif-li))),them[lif-1]); // as above but wraps to width
