...
function mytask() {
    ...
    for(i=0;i<16;i++) {
        ...
        if(ton[i]==1) {
            ...
            if(tramp[i]>25) tramp[i] = 25; // limit to 50
            frics[i] = (tramp[i] + 124) / 150;
        }

        dx[i] = dx[i] + (pointk[i] - x[i]) / g; //removed +0.5 before /
...
function aa_draw() { // draws the x array in the monome /grid/led/level/map format with anti-aliasing
    for(i=0;i<steps*cols;i++) them[i] = direction*b; //set all leds to 0 or 15 re:direction
