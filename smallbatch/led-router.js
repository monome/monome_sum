  ...
  // should create a call for /sys/info for apps that request it in case they don't respond to initialisation
	...
	if(a=="/manager/grid/led/set") { // i:<x>, i:<y>, i:<state>
		if(glob.gMeta==0) { // only one application so will be inlet==0 only
		  if(inlet==0) { // ignore messages from 2nd app
			if(x<glob.g1x) { // the press is within the range of the left most device
				outlet(0,"/manager/grid/led/set",x,y,z);
			}
			else outlet(1,"/manager/grid/led/set",(x-glob.g1x),y,z);
		  }
		}
	...
	if(a=="/manager/grid/led/map") { // i:<x-offset>, i:<y-offset>, i:[8 led/rows]
		if(glob.gMeta==0) { // only one application so will be inlet==0 only
		  if(inlet==0) { // need to ignore 2nd application
			if(x<glob.g1x) outlet(0,"/manager/grid/led/map",x,y,z,n,o,p,q,r,s,t); // in range of 1st device then echo out
			else outlet(1,"/manager/grid/led/map",(x-glob.g1x),y,z,n,o,p,q,r,s,t); // remove offset
		  }
		}
		if(glob.gMeta==1) { // 2 apps & 2 devices
		...
		if(glob.gMeta==2) { // landcape128 from 2 apps
			if(x==0 && y==0) { // only 1st quad messages are in range (no support for v-split 256)
				if(inlet==0) outlet(0,"/manager/grid/led/map",0,0,z,n,o,p,q,r,s,t);
				else outlet(0,"/manager/grid/led/map",8,0,z,n,o,p,q,r,s,t); // shift to right
			}
		}
		if(glob.gMeta == 3) { // portrait128/256 from 2 apps
			if(y==0) { // only 1st&2nd quad are in range
	...
	if(a=="/manager/grid/led/row") { // i:<x-offset>, i:<y-row>, i:<bitmask>, i:<bitmask2>
		if(glob.gMeta==0) { // only one application so use inlet==0 only
		  if(inlet==0) {
			if(glob.g1x==0) { // if there is only 1 device attached
				outlet(0,"/manager/grid/led/row",x,y,z,n);
			}
			...
			if(glob.g1x==16) { // if the 1st device attached is 16 wide
				...
				if(x==8) { // and the /led/row applies to the right quad
				...
				if(x>8) { // and the /led/row applies to the right grid
	...
	if(a=="/manager/grid/led/col") { // i:<x-col>, i:<y-offset>, i:<bitmask>, i:<bitmask2>
		if(glob.gMeta==0) { // only one application so will be inlet==0 only
		  if(inlet==0) { // need to ignore 2nd app return
			if(x<glob.g1x) outlet(0,"/manager/grid/led/col",x,y,z,n); // if first device send out unchanged
			else outlet(1,"/manager/grid/led/col",(x-glob.g1x),y,z,n); // else send out with x-offset
		  }
		}
		if(glob.gMeta==1) { // 2 apps & 2 devices
